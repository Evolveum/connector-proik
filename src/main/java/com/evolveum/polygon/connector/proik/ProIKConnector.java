...
//        objClassBuilder.addAttributeInfo(attrUidBuilder.build());
        ...
        attrModifyTimestampBuilder.setUpdateable(false); // read only
    ...
    private Uid createUser(Set<Attribute> attributes) {
        ...
        try {
            ...
            // see handleAdministrativeStatus
//            if (enable != null) {
...
//            }
            ...
            LOG.ok("createUser: tckn:{0}, adi:{1}, soyadi:{2}, kullaniciAdi:{3}, eposta:{4}, enable:{5}, " +
                            "Aktifmi:{6}, etkiAlanieh:{7}, etkiAlani:{8}",
                    uid, firstName, lastName, userName, emailAddress, enable, proUser.getAktifmi(), isDomainName, domainName);
    ...
    private Uid updateUser(Uid uid, Set<Attribute> attributes) {
        ...
        String userName = getStringAttr(attributes, Name.NAME, origUser.getKullaniciAdi());
...
//            }
    ...
    private ConnectorObject convertUserToConnectorObject(ProUser user) throws RemoteException {
        ...
        builder.setName(user.getKullaniciAdi());
